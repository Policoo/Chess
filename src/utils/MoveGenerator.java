...
public class MoveGenerator {
    ...
    public static List<int[]> generateMoves(int x, int y, Board board) {
    ...
    public static HashMap<int[], List<int[]>> generateMovesForColorToMove(Board board) {
        HashMap<int[], List<int[]>> moves = new HashMap<>();
        for (int y = 0; y < 8; y++) {
            for (int x = 0; x < 8; x++) {
                if (board.getTile(x, y) == null || !board.getTile(x, y).isColor(board.getColorToMove()) || board.getGameOver()) {
                    continue;
                ...
                switch (board.getTile(x, y).getType()) {
                    ...
                    case "q":
                        ...
                        break;
                    case "k":
                        List<int[]> kingMoves = generateKingMoves(x, y, board);
    ...
    public static List<int[]> generatePawnMoves(int x, int y, Board board) {
        List<int[]> moves = new ArrayList<>();
        int direction;
        String color = board.getTile(x, y).getColor();
        ...
        if (y + direction >= 0 && y + direction < 8 && board.getTile(x, y + direction) == null && board.isLegalMove(x, y, x, y + direction)) {
            int[] currentMove = new int[2];
            currentMove[0] = x;
            currentMove[1] = y + direction;
            moves.add(currentMove);
        }
        //if pawn has not yet moved, check if moving 2 tiles is possible and if the tile in front of you is empty
        if (!board.getTile(x, y).hasMoved() && y + (direction * 2) >= 0 && y + (direction * 2) < 8 && board.getTile(x, y + (direction * 2)) == null && board.getTile(x, y + direction) == null && board.isLegalMove(x, y, x, y + (direction * 2))) {
            int[] currentMove = new int[2];
            currentMove[0] = x;
            currentMove[1] = y + (direction * 2);
            moves.add(currentMove);
        }
        ...
        for (int leftOrRight : leftAndRight) {
            ...
            if (board.getTile(x + leftOrRight, y + direction) != null && !board.getTile(x + leftOrRight, y + direction).isColor(color) && board.isLegalMove(x, y, x + leftOrRight, y + direction)) {
                int[] currentMove = new int[2];
                currentMove[0] = x + leftOrRight;
                currentMove[1] = y + direction;
                moves.add(currentMove);
            }
            //check for en passant
            if (board.getTile(x + leftOrRight, y) != null && board.getTile(x + leftOrRight, y).isEnPassantPossible() && !board.getTile(x + leftOrRight, y).isColor(color) && board.isLegalMove(x, y, x + leftOrRight, y + direction)) {
                int[] currentMove = new int[2];
                currentMove[0] = x + leftOrRight;
                currentMove[1] = y + direction;
                moves.add(currentMove);
    ...
    public static List<int[]> generateBishopMoves(int x, int y, Board board) {
        List<int[]> directions = generateSlidingPieceDirections("b");
        return scanDirectionUntilCollision(x, y, directions, board);
    }

    public static List<int[]> generateKnightMoves(int x, int y, Board board) {
        List<int[]> directions = generateKnightDirections();
        return scanDirectionOnce(x, y, directions, board);
    }

    public static List<int[]> generateRookMoves(int x, int y, Board board) {
        List<int[]> directions = generateSlidingPieceDirections("r");
        return scanDirectionUntilCollision(x, y, directions, board);
    }

    public static List<int[]> generateQueenMoves(int x, int y, Board board) {
        List<int[]> moves = new ArrayList<>();
    ...
    public static List<int[]> generateKingMoves(int x, int y, Board board) {
        List<int[]> directions = generateSlidingPieceDirections("k");
        return scanDirectionOnce(x, y, directions, board);
    }

    public static List<int[]> checkCastling(String color, int x, int y, Board board) {
        List<int[]> castleMove = new ArrayList<>();
        ...
        if (board.getTile(x + 1, y) == null && board.getTile(x + 2, y) == null) {
            ...
            if (board.getTile(x + 3, y) != null && board.getTile(x + 3, y).isType("r") && board.getTile(x + 3, y).isColor(color) && !board.getTile(x + 3, y).hasMoved()) {
                //if move is legal
                if (board.isLegalMove(x, y, x + 1, y) && board.isLegalMove(x, y, x + 2, y)) {
                    int[] currentMove = new int[2];
                    currentMove[0] = x + 2;
                    currentMove[1] = y;
                    castleMove.add(currentMove);
                }
            }
        }
        //if the tiles between king and rook are empty
    ...
    public static List<int[]> scanDirectionOnce(int x, int y, List<int[]> directions, Board board) {
        List<int[]> moves = new ArrayList<>();
        String color = board.getTile(x, y).getColor();
        for (int[] direction : directions) {
            if (x + direction[0] >= 0 && x + direction[0] < 8 && y + direction[1] >= 0 && y + direction[1] < 8 && (board.getTile(x + direction[0], y + direction[1]) == null || !board.getTile(x + direction[0], y + direction[1]).isColor(color))) {
                if (board.isLegalMove(x, y, x + direction[0], y + direction[1])) {
                    int[] currentMove = new int[2];
                    currentMove[0] = x + direction[0];
                    currentMove[1] = y + direction[1];
                    moves.add(currentMove);
                }
            }
        }
    ...
    public static List<int[]> scanDirectionUntilCollision(int x, int y, List<int[]> directions, Board board) {
        List<int[]> moves = new ArrayList<>();
        String color = board.getTile(x, y).getColor();
        for (int[] direction : directions) {
            int currentX = x;
            ...
            while (currentX + direction[0] >= 0 && currentX + direction[0] < 8 && currentY + direction[1] >= 0 && currentY + direction[1] < 8 && (board.getTile(currentX + direction[0], currentY + direction[1]) == null || !board.getTile(currentX + direction[0], currentY + direction[1]).isColor(color))) {
                ...
                if (board.isLegalMove(x, y, currentX, currentY)) {
                    int[] currentMove = new int[2];
                    currentMove[0] = currentX;
                    currentMove[1] = currentY;
                    moves.add(currentMove);
                }
